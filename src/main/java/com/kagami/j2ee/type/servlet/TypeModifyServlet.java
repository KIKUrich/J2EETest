...
@WebServlet(name = "typeModifyServlet", value = "/type/modify")
public class TypeModifyServlet extends HttpServlet {
    ...
    public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        typeInfo.setTypeId(request.getParameter("type_id") == null ? 0 : Integer.parseInt(request.getParameter("type_id")));
        typeInfo.setTypeName(request.getParameter("type_name") == null ? "" : request.getParameter("type_name"));
        TypeInfo result = this.typeService.modifyTypeInfo(typeInfo);
        if (result == null)
            System.out.println("修改书籍类别失败！");
